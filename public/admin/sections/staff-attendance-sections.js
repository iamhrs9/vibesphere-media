    ...
    function bindStaffDom() {
        ...
        document.getElementById('addStaffForm')?.addEventListener('submit', async (e) => {
            ...
            const newStaff = {
                ...
                whatsappNumber: document.getElementById('staffWhatsapp').value
            };
            try {
            ...
            btn.innerText = 'Create Staff ID';
                    ...
                    return `
                        <div id="staffProfileEditPanel" class="toolbar-card" style="display:none;margin-top:18px;">
                            ...
                            <div class="toolbar-field">
                                    ...
                                    <input type="text" id="staffEditWhatsapp" value="${escapeHtml(staff.whatsappNumber || '')}" placeholder="e.g., 919876543210">
                                </div>
                            </div>
                ...
                window.saveStaffProfileEdits = async function (staffId) {
                    const payload = {
                        ...
                        whatsappNumber: document.getElementById('staffEditWhatsapp')?.value || ''
                    };
                ...
                function handleStaffFilterChange() {
                    ...
                    const isSearchEvent = e && e.type === 'input';
                ...
                window.openStaffAttendanceView = function (email) {
                    const staffSearch = document.getElementById('staffSearchMirror');
                    const attSearch = document.getElementById('attendanceSearchMirror');
                    const attWsSearch = document.getElementById('attendanceSearchMirrorWorkspace');
                    if (staffSearch) staffSearch.value = email;
                    if (attSearch) attSearch.value = email;
                    if (attWsSearch) attWsSearch.value = email;
                ...
                async function fetchStaffData() {
                    ...
                    // ====================================================
                    // ⚡ SWR STEP 1: Serve stale cache INSTANTLY if available
                    // ====================================================
                    const cacheKey = `staff_p${adminUiState.staffPage}_l${adminUiState.staffLimit}_s${adminUiState.staffStatus}_q${document.getElementById('staffSearchMirror')?.value || ''}`;
                    const cached = window.adminDataCache?.[cacheKey];

                    if (cached) {
                        // Render the cached data immediately — zero network wait
                        adminUiState.staffPage = cached.pagination?.page || 1;
                        adminUiState.staffTotalPages = cached.pagination?.totalPages || 1;
                        adminUiState.staffTotal = cached.pagination?.total || 0;
                        adminUiState.staffRecords = cached.staff || [];
                        renderStaffDirectoryViews(adminUiState.staffRecords);
                        ...
                        updateTopbarStatus('Staff directory (cached)');
                        // Show a subtle "refreshing" indicator without blocking the UI
                        if (staffTbody) staffTbody.style.opacity = '0.65';
                    } else {
                        // No cache — show rich skeleton loaders while we wait
                        if (staffTbody) staffTbody.innerHTML = renderLoadingRows(6, adminUiState.staffLimit || 8);
                        if (grid) grid.innerHTML = renderSkeletonCards(4);
                        if (perfTbody && shouldRefreshPerformance) perfTbody.innerHTML = renderLoadingRows(5, 3);
                    }

                    // ====================================================
                    // ⚡ SWR STEP 2: Always fetch fresh data in background
                    // ====================================================
                    try {
                        const currentSearchValue = document.getElementById('staffSearchMirror')?.value || '';
                        ...
                        const latestSearchValue = document.getElementById('staffSearchMirror')?.value || '';
                        if (currentSearchValue !== latestSearchValue) return; // Discard stale response

                        if (!staffData.success) throw new Error(staffData.message || 'Failed to load staff directory.');

                        // ====================================================
                        // ⚡ SWR STEP 3: Store fresh data in cache, re-render
                        // ====================================================
                        if (!window.adminDataCache) window.adminDataCache = {};
                        window.adminDataCache[cacheKey] = staffData;
                        ...
                        // Restore opacity if we faded it for the "refreshing" indicator
                        if (staffTbody) staffTbody.style.opacity = '';
                        ...
                        if (!cached) {
                            ...
                            if (staffTbody) staffTbody.style.opacity = ''; // Restore faded cache view
                        }
                ...
                async function fetchAdminAttendance(context = 'section') {
                    ...
                    try {
                        const currentSearchValue = document.getElementById(context === 'workspace' ? 'attendanceSearchMirrorWorkspace' : 'attendanceSearchMirror')?.value || '';
                        ...
                        const latestSearchValue = document.getElementById(context === 'workspace' ? 'attendanceSearchMirrorWorkspace' : 'attendanceSearchMirror')?.value || '';
                        if (currentSearchValue !== latestSearchValue) return; // Discard outdated response to prevent race condition
