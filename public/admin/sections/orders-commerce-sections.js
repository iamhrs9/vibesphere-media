(function () {
    ...
    async function fetchOrders() {
        try {
            ...
            if (!orderList.length) {
                tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;color:#64748b;padding:24px;">${searchTerm ? 'No orders matching "' + searchTerm + '"' : 'No orders found.'}</td></tr>`;
                return;
            }
            ...
            tbody.innerHTML = orderList.map((order) => {
                ...
                return `
                    <tr id="order-row-${escapeHtml(order.orderId || '')}">
                        ...
                        <td>
                            <div style="font-size:0.82rem;color:#0f172a;font-weight:600;white-space:nowrap;">${formatAdminDateTime(orderTs)}</div>
                        </td>
                        <td>
    ...
    async function fetchSmmOrders() {
        ...
        try {
            ...
            tbody.innerHTML = orderList.map((order) => {
                ...
                const smmOrderTs = getOrderTimestamp(order);
                ...
                return `
                    <tr id="smm-order-row-${escapeHtml(order.orderId || '')}">
                        ...
                        <td>
                            <div style="font-size:0.82rem;color:#0f172a;font-weight:600;white-space:nowrap;">${formatAdminDateTime(smmOrderTs)}</div>
                        </td>
                        <td>
                            <div style="font-weight:700;color:#0f172a;">${escapeHtml(order.customerName || 'Unknown Customer')}</div>
                            <div style="margin-top:4px;font-size:0.82rem;color:#64748b;">${escapeHtml(order.email || '—')}</div>
